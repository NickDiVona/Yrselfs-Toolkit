# Yrself's Toolkit

Raid utilities for Theatre of Blood and Tombs of Amascut.

## Features

### Theatre of Blood - Maiden Freeze Chance

Displays an infobox while you're in Maiden (or the Ver Sinhaza area (ToB Bank)) showing the probability your current magic level and gear can freeze a Nylo in the Maiden's room. Accounts for magic prayers, void, gear, etc.

### Tombs of Amascut - Lobby Checks

Shows an overlay on the ToA entrance while you're in the lobby. Each check can be toggled independently in the plugin config under the **ToA** section.

**Check Thralls** - verifies you have everything needed to summon thralls:

- Spellbook is Arceuus
- Book of the Dead is carried (inventory or equipped)
- Fire, Blood, and Cosmic runes present (Aether counts as Cosmic)

**Check Death Charge** - verifies you have the runes to cast Death Charge:

- Death, Blood, and Soul runes present (Aether counts as Soul)

Runes are detected loose in your inventory, stored inside a regular Rune Pouch, or a Divine Rune Pouch.

## Thanks

I appreciate the help I received from the following people when making this plugin:

- Coley
- Bombology
- based batt
- Gabby
